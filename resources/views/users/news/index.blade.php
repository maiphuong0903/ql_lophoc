...
@section('content')
    ...
    <div class="grid grid-cols-4 bg-gray-100 h-full z-10">
        <div class="col-span-3">
           <div class="border border-1 px-5 py-6 bg-white font-medium">
                <h1>Bảng tin</h1>
           </div>
           <div class="my-10 mx-32">
                ...
                @forelse ($newsFeeds as $newsFeed)       
                    ...
                    <div class="bg-white rounded-lg shadow-sm py-5 px-5 mt-5 news-feed-container">
                        ...
                        <div class="flex flex-1 justify-between items-center">
                            <div class="flex flex-1 gap-3">
                                @if($newsFeed->author->avatar)
                                    <img src="{{ $newsFeed->author->avatar }}" alt="Avatar" class="w-12 h-12 rounded-full">
                                @else
                                   <img src="{{ asset('images/avatar.jpg') }}" alt="" class="object-cover w-12 h-12 rounded-full">
                                @endif                              
                                <div>
                            ...
                            <div class="relative">
                                ...
                                @if (auth()->user()->role == 2 || $newsFeed->created_by == auth()->user()->id)             
                        ...
                        <div class="hidden comment-item">
                            <div class="flex flex-1 justify-between items-center mt-3">
                                <div class="flex gap-2">               
                                    @if($newsComment->user->avatar)
                                        <img src="{{ $newsComment->user->avatar }}" alt="Avatar" class="w-12 h-12 rounded-full">
                                    @else
                                        <img src="{{ asset('images/avatar.jpg') }}" alt="" class="object-cover w-12 h-12 rounded-full">
                                    @endif
                                    <div>
                                ...
                                @if (auth()->user()->role == 2 || $newsComment->user_id == auth()->user()->id)                   
                                    <button type="button" class="deleteNewsComment" data-news-feed-id="{{ $newsFeed->id }}" data-newsComment-id="{{ $newsComment->id }}" data-modal-target="deleteModal" data-modal-toggle="deleteModal">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.deleteNewsFeed', function(){
            $('.menu-news').addClass('hidden');
            $('#deleteModal').removeClass('hidden');
            let newsId = $(this).data('news-id');      
            let formAction = "{{ route('class.newsfeed.destroy', ':newsfeedId') }}".replace(':newsfeedId', newsId);             
        ...
        $('[data-modal-toggle="deleteModal"]').click(function(){
         ...
         // Mở form xóa comment 
         $(document).on('click', '.deleteNewsComment', function(){
            $('#deleteModal').removeClass('hidden');
            let newsFeedId =  $(this).data('news-feed-id'); 
            let commentId = $(this).data('newscomment-id');     
           
            let formAction = "{{ route('class.newsfeed.comment.destroy', [':newsfeedId', ':commentId']) }}";     
            formAction = formAction.replace(':newsfeedId', newsFeedId);
