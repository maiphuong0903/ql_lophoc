    ...
    <div class="mt-24 mx-5">
        ...
        <div class="mt-5 mb-5 flex gap-3 items-center">
            <div class="relative flex justify-between items-center w-1/2">
                <form action="{{ route('class.exams.show', ['id' => $classRoom->id, 'examId' => $exam->id]) }}" method="GET" class="w-full">
            ...
            <div class="flex items-center gap-2 bg-blue-500 py-2 px-4 text-white rounded-md justify-center">         
                <a href="{{ route('class.exams.show', ['id' => $classRoom->id, 'examId' => $exam->id, 'status-exam' => 'all']) }}">Tất cả</a>
            </div>
        
            <div class="flex items-center gap-2 bg-gray-200 py-2 px-4 rounded-md justify-center">         
                <a href="{{ route('class.exams.show', ['id' => $classRoom->id, 'examId' => $exam->id, 'status-exam' => 'markscore']) }}">Đã nộp</a>
            </div>
        
            <div class="flex items-center gap-2 bg-gray-200 py-2 px-4 rounded-md justify-center">         
                <a href="{{ route('class.exams.show', ['id' => $classRoom->id, 'examId' => $exam->id, 'status-exam' => 'notScore']) }}">Chưa nộp</a>
            </div>  
            <a href="{{ route('class.exams.printExcel', ['id' => $classRoom->id, 'examId' => $exam->id]) }}" class="border border-gray-300 flex justify-center py-2 px-4 rounded-lg bg-white cursor-pointer">
                ...
                <tbody>
                    @forelse ($students as $key => $student)
                        <tr class="odd:bg-white even:bg-gray-50 border-b">
                            <td class="px-6 py-4">{{ ($students->currentPage() - 1) * $students->perPage() + $key+1 }}</td>
                            <td class="px-6 py-4">{{ $student->name }}</td>
                            <td class="px-6 py-4">{{ $student->answerExams()->where('exam_id', $exam->id)->first()->pivot->score ?? 'Chưa nộp'}}</td>
                            <td class="px-6 py-4">
                                {{ $student->answerExams()->where('exam_id', $exam->id)->first() ?
                                $student->answerExams()->where('exam_id', $exam->id)->first()->created_at->format('Y-m-d H:i') : '' }}
                            </td>
                            <td class="px-6 py-4">
                                @if (isset($student->answerExams()->where('exam_id', $exam->id)->first()->pivot->score))         
                                    <a href="{{ route('class.exams.detail', ['id' => $classRoom->id, 'examId' => $exam->id, 'studentId' => $student->id]) }}" class="border px-4 py-1.5 rounded-md hover:bg-blue-500 hover:text-white">Chi tiết</a>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="text-center p-5">Không tồn tại học sinh nào</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
